...
#'   Default: here("exp1","doc","chimps").
#' @param prefix Filename prefix.
#'   Default: "detected_blinks_".
#' @param width,height PDF size in inches.
#'
#' @return Invisibly returns the file path.
#' @export
save_detected_blinks_pdf <- function(
  ...
  participants <- unique(na.omit(df$participant_name))
  
  participant <- if (length(participants) == 1) {
    tolower(as.character(participants))
  } else {
    paste0(length(participants), "_participants")
  }
