...
for(i in 1:length(filenames)){
  ...
  # Read Data ---------------------------------------------------------------
  raw <- readRDS(here("exp2", "data", "raw_clean_blink", folder, filename))
  ...
  if (folder == "human_calibration_9p") {
    df <- df |>
      unite(col = "session_trial", c("participant_name", "recording_name", "trial"), remove = FALSE)
  } else {
    df <- df |>
      unite(col = "session_trial", c("recording_name", "trial"), remove = FALSE)
  }
  ...
  # Add human calibration info
  df_tot <- df_tot |> 
      left_join(df |> select(session_trial, participant_name) |> distinct()) |> 
    distinct()
  
  df_tot$participant_name <- df_tot$participant_name |>
    tolower() |>
    gsub("1|_", "", x = _)
  
  # 6) Save file
  saveRDS(df_tot, here("exp2", "data", "preproc", folder, filename), compress = "xz")
