...
acc_tot <- df_human_9p |> 
  bind_rows(df_ape_2p, df_alex_5p)

# Summarize Data ----------------------------------------------------------
## dplyr::summarize Accuracy ----
acc_sum_1 <- mean_sd(acc_tot, c("condition", "filename"), "acc_visd") # across trials and positions, per participant
acc_sum_2 <- mean_sd(acc_tot, c("condition", "filename", "position"), "acc_visd") # across trials, per position, per participant
acc_sum_3 <- mean_sd(acc_tot, c("condition", "position"), "acc_visd") # across trials and participants, per position
acc_sum_4 <- mean_sd(acc_tot, c("condition"), "acc_visd") # across trials and participants, per position

# Plot Data ---------------------------------------------------------------
df_plot <- acc_sum_1 |> 
  mutate(
    participant = filename |>
      str_remove("^accuracy_") |>
      str_remove("\\.rds$") |>
      str_to_title(),
    
    condition = case_when(
      condition == "alex_calibration_5p"  ~ "Other Ape Calibration\n(5 Points)",
      condition == "ape_calibration_2p"   ~ "Own Ape Calibration\n(2 Points)",
      condition == "human_calibration_9p" ~ "Human Calibration\n(9 Points)",
      TRUE ~ condition
    ),
...
n_participants <- n_distinct(df_plot$participant)
pal <- scales::hue_pal(l = 65, c = 100)(n_participants)

png(here("exp2", "img", "exp2_plot1.png"), width = 1920, height = 1080, res = 250)
ggplot(df_plot, aes(x = condition, y = mean_acc_visd,
...
df_sum <- df_plot |> 
  ...
  summarise(
    n = n(),
    mean = mean(mean_acc_visd, na.rm = TRUE),
    sd = sd(mean_acc_visd, na.rm = TRUE),
    se = sd / sqrt(n),
...
png(here("exp2", "img", "exp2_plot2.png"), width = 1920, height = 1080, res = 250)
ggplot(df_sum, aes(x = condition, y = mean, group = 1)) +
...
## Trial Contributions ----
acc_tot |> 
  drop_na(acc_visd, recording_name) |> 
  group_by(trial, recording_name, condition) |> 
  count() |> 
  filter(n==2)
...
plot_distribution_rq1(data = acc_sum_1 |>
...
# Plot per position
acc_sum_2$position <- factor(acc_sum_2$position, levels = c("center", "topleft", "top", "topright", "botleft", "bottom", "botright"))
acc_sum_2$age_group <- factor(acc_sum_2$age_group, levels = c("4", "6", "9", "18", "adult", "apes (2-point ape calibration)", "apes (9-point human calibration)"))
...
ggplot(acc_sum_2, aes(x = position, y = mean_acc_visd, color = age_group)) +
...
# Plot the session-wise average
n_apes_session <- acc_sum_3_session |> pull(filename) |> unique() |> length()

png(here("img", "acc_session_2.png"), width = 2048, height = 1152, res = 300)
ggplot(acc_sum_3_session |> group_by(session) |> dplyr::summarize(sd_acc_visd = sd(mean_acc_visd, na.rm = T),
...
# Create the plot
ggplot(acc_sum_2 |>  mutate(position = factor(position, levels = c("center", "top", "topleft", "topright", "bottom", "botleft", "botright"))), 
...
N <- acc_sum_1 |> drop_na(filename) |>  select(age_group, filename) |> distinct() |> group_by(age_group) |> count() |> ungroup()
