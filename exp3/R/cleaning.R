#' Rename stimulus identifiers into standardized format
#'
#' This function standardizes raw stimulus labels from an eye-tracking / experimental
#' paradigm into a consistent naming scheme. It handles checkflake (pre/mid/post)
#' stimuli, cajotask conditions (still/move/at), and preserves selected control trials.
#'
#' @param x Character vector of raw stimulus names.
#'
#' @return Character vector with renamed stimulus identifiers.
#'
#' @details
#' The function applies the following transformations:
#'
#' \itemize{
#'   \item \strong{No change}: `cajopilot` trials and gaze control stimuli
#'         (directgaze/shift left/right) are kept unchanged.
#'
#'   \item \strong{Checkflake (popflake) stimuli}:
#'         `pre_`, `mid_`, and `post_` prefixes are converted into a unified
#'         `*_popflake_*` structure while preserving the trial index.
#'         Spatial labels are simplified:
#'         `top_left` → `topleft`, `bot_right` → `botright`, etc.
#'         `center_center` → `center`.
#'
#'   \item \strong{Cajotask - still}:
#'         Converted to `ID_object_1s_top/bottom` depending on spatial position.
#'
#'   \item \strong{Cajotask - move}:
#'         Converted to `ID_object_5s_top/bottom`.
#'
#'   \item \strong{Cajotask - at}:
#'         Converted to `ID_pinwheel_30s_center`.
#' }
#'
#' @examples
#' rename_stimulus(c("mid_1_checkflake_30s_bot_left",
#'                   "12_cajotask_move_..._top_first",
#'                   "directgaze_right"))
#'
#' @export
rename_stimulus <- function(x) {
  
  case_when(
    
    # pilot study stimuli (not analysed for this project later)
    # collapse all cajo2pilot trials into one category
    str_detect(x, "cajo2pilot|directgaze_|shift_|objectgaze_") ~ "cajo2pilot",

    # popflake
    str_detect(x, "^(pre|mid|post)_") ~ {
      
      x2 <- str_replace(
        x,
        "^(pre|mid|post)_([0-9]+)_checkflake_",
        "\\2_popflake_"
      )
      
      x2 <- str_replace(x2, "_center_center", "_center")
      x2 <- str_replace(x2, "bot_left", "botleft")
      x2 <- str_replace(x2, "bot_right", "botright")
      x2 <- str_replace(x2, "top_left", "topleft")
      x2 <- str_replace(x2, "top_right", "topright")
      
      x2
    },
    
    # still (object image)
    str_detect(x, "cajotask_still") ~ {
      id <- str_extract(x, "^[0-9]+")
      pos <- ifelse(str_detect(x, "top"), "_top", "_bottom")
      paste0(id, "_object_1s", pos)
    },
    
    # move (object move)
    str_detect(x, "cajotask_move") ~ {
      id <- str_extract(x, "^[0-9]+")
      pos <- ifelse(str_detect(x, "top"), "_top", "_bottom")
      paste0(id, "_object_5s", pos)
    },
    
    # pinwheel (at)
    str_detect(x, "cajotask_at") ~ {
      id <- str_extract(x, "^[0-9]+")
      paste0(id, "_pinwheel_30s_center")
    },
