...
# Function ----------------------------------------------------------------
source(here("exp3", "R", "blink.R"))

# Adjust Parameter --------------------------------------------------------
folder_name <- "6to18mo" # "4mo", "6to18mo"
filenames <- list.files(path = here("exp3", "data", "raw_clean", folder_name))
...
for(i in 1:length(filenames)){
  ...
  ## Read Data ----
  raw <- readRDS(here("exp3", "data", "raw_clean", folder_name, filenames[nr]))
  df <- raw
  
  ## Correct data (if export mistake)
  if(df$pupil_diameter_left |> is.character()){
    df$pupil_diameter_left <- as.numeric(gsub(",", ".", df$pupil_diameter_left)) 
    print("Pupil diameter left now numeric.")
  }
  
  ## Save Plot (Pre Smoothing)
  png(here("exp3", "doc", folder_name, paste0(filenames[i] |> str_replace(".rds",""), "_1_pupil_presmooth", ".png")), width = 2048, height = 1152, res = 300)
  p1 <- ggplot(df |>
  ...
  png(here("exp3", "doc", folder_name, paste0(filenames[i] |> str_replace(".rds",""), "_2_pupil_postsmooth", ".png")), width = 2048, height = 1152, res = 300)
  p2 <- ggplot(df |>
                 mutate(time = cumsum(c(0, diff(recording_timestamp)))) |> 
                 mutate(time = cumsum(time)/1000) |> 
                 filter(!(presented_stimulus_name %in% c("Eyetracker Calibration", ""))),
  ...
  png(here("exp3", "doc", folder_name, paste0(filenames[i] |> str_replace(".rds",""), "_3_pupil_postsmooth_postoutlier", ".png")), width = 2048, height = 1152, res = 300)
  p3 <- ggplot(df |>
                 mutate(time = cumsum(c(0, diff(recording_timestamp)))) |> 
                 mutate(time = cumsum(time)/1000) |> 
                 filter(!(presented_stimulus_name %in% c("Eyetracker Calibration", ""))),
  ...
  for (p in unique(df$participant_name)) {
    
    df_sub <- df |> dplyr::filter(participant_name == p)
    
    plots_sub <- plot_blinks(
      df_sub,
      eye = "both",
      flank_n = 5,
      min_run = 2,
    ...
    save_detected_blinks_pdf(plots_sub, df_sub, out_dir = here::here("exp3", "doc", paste0("blinks_", str_replace(filenames[nr], ".rds", ""), "_", p, ".pdf")))
    rm(df_sub, plots_sub)
  }
  
  # Write data
  out_rds <- here("exp3", "data", "raw_clean_blink", folder_name, filenames[nr])
  saveRDS(df, out_rds, compress = "xz")
  
  print(i)
  print(df |> select(recording_name, average_calibration_accuracy_degrees) |> distinct())
}
